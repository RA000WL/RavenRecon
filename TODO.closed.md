...
- Owner: builder
- Problem: SDK v1 dependencies order execution but never flow data (detect.go:187-188, ARCHITECTURE.md:2369); persist blocked since v0.2 (no asset store/graph traversal). Concrete failing need: authz.idor rule inexpressible on v1 (needs PriorFindings + GraphView). Blocked Auth/AuthZ/Business-logic packs.
- Fix: SDK surface (api.go APIMajor 1→2, context.go PriorFindings []asset.Finding + GraphView GraphQuerier[Neighbors,Path] read-only over Snapshot.Relationships+Assets, clone helper depth extended, graphView map index built once per Run); engine wiring (engine.go level barrier collects findings into PriorFindings for next levels, GraphView wraps snapshot); cache coherence (record.go SchemaVersion 2→3, fingerprintSnapshot gains GraphDigest (hash of sorted Relationship.ID()), ruleKey gains graph_digest (graph hash + PriorFindings digest), old records decode-rejected); minimal auth pack (internal/detect/packs/auth auth.jwt.none-alg, stateless, hermetic, bounded, demonstrates PriorFindings+GraphView, CheckAPIVersion(2,0)); testdata/api_v2.golden regenerated same change via go test -update; AllStages 12 unchanged, AllPacks not bumped for this SDK change (auth probe not counted). See api.go reopening note and ARCHITECTURE.md SDK v2 doc.
- Verification: gates: gofmt -w, go vet ./..., go build ./..., go test ./... -count=1 (all 28+ packages pass, discovery ~112s), go test -race ./internal/detect/... green, golden diff api_v1→api_v2 (APIMajor 2, PriorFindings, GraphView, GraphQuerier, SchemaVersion 3, graph_digest), auth pack tests (PriorFindingsVisible, GraphViewNeighborsPath, CacheColdWarmParity, Determinism, UsesOnlyExportedSurface), context_isolation updated for GraphView/PriorFindings, tampered cache recompute still surfaces diagnostic.
- 4-step gate: (1) failing need authz.idor+auth.jwt.none-alg inexpressible on v1 (no Context field), (2) proposal names exact symbols (Context.PriorFindings, Context.GraphView/GraphQuerier.Neighbors/Path, Snapshot.graph_digest, SchemaVersion 2→3, APIMajor 1→2), (3) maintainer approval documented in api.go reopening decision bumping APIMajor, (4) golden regeneration + version bump same commit (api_v2.golden via -update, 9 behavior contracts + compat golden green).
- Known limitations: persistent store file layout beyond in-memory GraphView deferred to next batch if needed for diff; Business-logic workflow pack, continuous monitoring daemon, browser/screenshots not in scope (v2.2 next batch, v2.3).
- Fix round (2026-09-03, review ses_f99bfb847ffeijM7lOF3TcDyLF REQUEST CHANGES): MEDIUM-1 cloneContextForRule deep-clones PriorFindings Metadata map layer (maps.Clone, nil-safe) + doc + TestCloneContextForRuleDepth nested assertion; MEDIUM-2 ARCHITECTURE.md Known-limitations bullet rewritten past-tense (in-memory-only view, no persistent store); LOW-1 api.go + ARCHITECTURE.md 4-step notes name only testdata/api_v2.golden (phantom report-golden refs removed); LOW-2 ruleKey graph_digest unconditional (fail closed) + TestRuleKeyInputs digest-sensitivity case. Status stays IN PROGRESS — awaiting orchestrator VERIFY.

- Close-out (2026-09-03, orchestrator): fix round verified (nested Metadata deep-clone + credible negative control; past-tense Known-limitations bullet; phantom golden refs removed; unconditional graph_digest + digest-sensitivity pin). Tier C docs completed by orchestrator (README SDK v2 + auth-probe notes; ROADMAP v2.2 status → In progress, Batch 1 landed). v2.2 milestone stays open (persistent store + `ravenrecon diff` pending).

### NF-7 (INFO) — README Status version header diverges from `internal/version` constant (docs)
- Status: VERIFIED — bulk verify 2026-08-27 (reviewer APPROVE): README header unversioned, version constant 2.0.0 source of truth
- Reporter: builder
- Owner: (unassigned)
...
- Status: VERIFIED — bulk verify 2026-08-27: utf8.ValidString rejection + >=0x80 percent-encode; ID stable across JSON round-trip (red→green proven)
...
- Status: VERIFIED — bulk verify 2026-08-27: URL-substring heuristics removed, re-graded Information/0.5; benign fixtures zero findings
...
- Status: VERIFIED — bulk verify 2026-08-27: IndicatorCookie name-only; value branch removed; doc parity
...
- Status: VERIFIED — bulk verify 2026-08-27: all 14 pack detectors honest overflow with subjects_dropped+truncated metadata
...
- Status: VERIFIED — bulk verify 2026-08-27: HasSuffix path-only, mapbox no longer fires
...
- Status: VERIFIED — bulk verify 2026-08-27: year-shaped and pagination excluded from IDOR; graphql split signals
...
- Status: VERIFIED — bulk verify 2026-08-27: redactURLForError clears userinfo before embedding; credential substrings absent
...
- Status: VERIFIED — bulk verify 2026-08-27: validateFingerprint rejects Group <1 or >NumSubexp()
...
- Status: VERIFIED — bulk verify 2026-08-27: WithValue back-dates FirstSeen to min
...
- Status: VERIFIED — bulk verify 2026-08-27: ~15 LOW residuals closed — spot-check 5/15 passed with dedicated tests (per verifier)
...
- Status: VERIFIED — bulk verify 2026-08-27: INFO wave — docs/comments fixed, accepted-as-is sub-items noted
- Reporter: reviewer (deep-pass 2026-08-25)
- Owner: builder
...
- Status: VERIFIED — bulk verify 2026-08-27: TLS SAN host expansion wired, hermetic tests, dedup vs corpus
...
- Status: VERIFIED — bulk verify 2026-08-27: naabu port discovery bounded capture, cache op, flags; fake-runner tests
- Reporter: builder (orchestrator dispatch, NEW-95 follow-up enhancement wave)
- Owner: builder (this session)
...
- Status: VERIFIED — bulk verify 2026-08-27: multi-target fan-out bounded WaitGroup+semaphore, per-target ScanConfig capture, sequential+parallel proofs
