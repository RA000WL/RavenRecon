...
| v2.1 | Reliability & Observability Closure | ✅ Complete (closed 2026-09-03 — NEW-96..109 VERIFIED, takeover + FP/FN + 10k perf + ASN-mapping groundwork; followed by NEW-118..133 verification wave: JS bodies, reflection, ports, takeover-confirm, chunking, session, DNS types, diff/handoff/config/crtsh/mistake-paths) |
| v2.2 | Graph & Inter-Rule Dataflow | 🔄 In progress — SDK v2 (APIMajor 1→2, PriorFindings+GraphView, SchemaVersion 2→3, api_v2.golden, auth probe) landed and verified (NEW-117); `ravenrecon diff` landed report-to-report (no baseline store — deviation recorded below); persistent asset store still pending. See §v2.2. |
...
Status: ✅ Complete (closed 2026-09-03; NEW-96..109 VERIFIED and archived; followed by the NEW-118..133 verification wave on top).
...
Status: 🔄 In progress — Batch 1 (SDK reopening: APIMajor 1→2, PriorFindings+GraphView, SchemaVersion 2→3, api_v2.golden, auth probe) landed and verified; `ravenrecon diff` landed report-to-report (deviation: no filesystem baseline store — diff compares report JSON exports, no `store/<target>/<runID>/` layout; persistent asset store still pending); SDK reopening follows the 4-step gate below.
...
- **`Context.PriorFindings` + `GraphView`** — `Context` gains `PriorFindings []asset.Finding` (findings from earlier levels in the same run, dependency-ordered) and `GraphView` (read-only traversal over `Snapshot` relationships + `PriorFindings` subjects — `Neighbors(identity)`, `Path(from,to)` reachability test; bounded, no mutation, backed by the normalized `Snapshot` maps; documented as the ONLY inter-rule dataflow; `cloneContextForRule` depth updated to clone the view handle immutably).
...
- **Business-logic workflow mapping** — `internal/detect/packs/bizlogic`: `bizlogic.workflow.state-transition` (multi-step dataflow across rules: `PriorFindings` from `authz.*` + `apis.*` + `web.*` to map state-transition edges via `GraphView.Path` host-reachability + correlated-not-traversed pairing (see NEW-144 design); Informational, workflow-state labels, never exploitation).
...
Status overview reflects the current shippable state: **8 packs / ~31 rules / AllStages 12** (`internal/detect/packs/*`: apis, auth, cloud, dnsrec, js, takeover, triage, web). v2.2 `SchemaVersion 2→3` / `APIMajor 1→2` was the first cache-invalidating bump since v2.0; `AllStages` stays 12 through v2.3 (new capabilities reuse the results channel + `GraphView`, no new `StageName`).
