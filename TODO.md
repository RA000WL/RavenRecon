...
- Status: IN PROGRESS (orchestrator, 2026-08-24) — Batches 1-4 VERIFIED; Batch 5 IMPLEMENTED (pending review/orchestrator verification)
- Reporter: master
...
- Batch 3 — JS pack (builder ses_fcca87426ffeic3Hb5mNZim8Yn 2026-08-24 VERIFIED; reviewer ses_fcc9fdd2dffe4SxPbDgkT28jNR APPROVE + polish ses_fcc99a837ffetIYkj6z4ckBysV — APPROVE): internal/detect/packs/js (doc.go, helpers.go, domxss.go, postmessage.go, prototypepollution.go, rules.go) exporting Rules() with CheckAPIVersion(1,0) → 3 rules js.dom.xss / js.postmessage.no-origin-check / js.prototype.pollution (each <85 lines, deterministic synthetic fixtures via URL-derived jsintel parse, context-honoring per-iteration, RequiredAssetTypes javascript gated, Config deterministic via sortedKeys, bounded at 256, PriorityHigh/StatusOpen, CategoryJavaScript/Information, MethodDetection evidence, observed-js subjects, stdlib-only, no regex, jsintel.NewParser reuse bounded 8 MiB); pipeline seam internal/pipeline/adapt/detect.go LoadJsPack / NewDetectStageWithJsPack / NewDetectStageWithAllPacks (ValidateRule→Register deepCopy→Validate→Seal, AllStages 12 unchanged, nil→empty unchanged); tests hermetic: packs/js/js_test.go (13 tests) + adapt/detect_js_test.go (3 seam tests mirroring web pack), golden internal/detect/packs/js/testdata/js_report.golden byte-stable 2 runs equal, + polish (doc honesty, dead Join removal); gates gofmt/go vet/go build green, go test ./... 29/29 pass (discovery 112s), go test -race ./internal/detect/packs/js green, api_v1.golden untouched; orchestrator VERIFIED re-running gates.
- Batch 4 — APIs pack (builder ses_fcc8ead5affe..., 2026-08-24 IMPLEMENTED + REVIEWED + VERIFIED (reviewer ses_fcc75a27bffeU8K2WzJVpIbPHs APPROVE 2026-08-24; orchestrator VERIFIED: re-ran gofmt/vet/build/test/race + surface golden, no exported drift)): internal/detect/packs/apis (doc.go, helpers.go, openapi.go, restidor.go, graphql.go, rules.go) exporting Rules() with CheckAPIVersion(1,0) → 3 rules api.openapi.exposed / api.rest.idor-indicator / api.graphql.introspection (each <100 lines, deterministic endpoint fixtures, context-honoring per-endpoint, RequiredAssetTypes endpoint gated, Config deterministic via sortedKeys, bounded at 256, PriorityInfo/StatusOpen, CategoryInformation/API, MethodDetection evidence, observed-endpoint subjects, stdlib-only); pipeline seam internal/pipeline/adapt/detect.go LoadApisPack / NewDetectStageWithApisPack / extended NewDetectStageWithAllPacks to 11 (5 web + 3 js + 3 apis) via ValidateRule→Register deepCopy→Validate→Seal, AllStages 12 unchanged, nil→empty unchanged; tests hermetic: packs/apis/apis_test.go (13 tests: CheckAPIVersion, LoadsThroughSDK deepCopy+Seal, MetadataDepsCompat, RequiredAssetTypesSkipHonestly, DetectorsHonorContext, FailuresIsolated panic→failed, CacheColdWarmParity same FindingsTruncated, DeterminismGolden via internal/golden -update byte-stable 2 runs equal, per-rule emission, ConfigDeterministic) + adapt seam detect_seam_test.go (3 tests: WithJsPack, WithAllPacks 11, LoadJsPack) now enabled via rename from detect_js_test.go (removes _js_ token, IgnoredGoFiles empty, AllPacks 11 verified with host+endpoint+js+evidence); goldens internal/detect/packs/apis/testdata/apis_report.golden byte-stable; gates gofmt/go vet/go build green, go test ./... 31/31 pass, go test -race ./internal/detect/... ./internal/pipeline/adapt/... green, api_v1.golden untouched; dead strings.Join lines removed from apis/openapi.go:81 restidor.go:50 graphql.go:83.
- Batch 5 — Cloud pack (builder subagent of ses_fcc66478dffe7x3kh0v8FFZfAm, 2026-08-24) IMPLEMENTED, gates green (review/verification pending): internal/detect/packs/cloud (doc.go, helpers.go, shapes.go, keys.go 87L, buckets.go 73L, firebase.go 92L, rules.go — each <100 lines) exporting Rules() with CheckAPIVersion(1,0) → 3 INFORMATIONAL-only rules per research Q5 verdict: cloud.aws.key-indicator / cloud.bucket.url / cloud.firebase.indicator (CategoryCloud/PriorityInfo/StatusOpen, MethodDetection evidence, observed-corpus subjects only; descriptions explicitly disclaim validity/publicness/exposure claims — no live verification per §0.1). Conservative FP posture: exact provider storage-host shapes (s3.amazonaws.com path-style+virtual-hosted, storage.googleapis.com, blob.core.windows.net); hand-rolled whole-token AKIA/ASIA access-key shape scanner with case-sensitive EXAMPLE-convention suppression (mirrors secrentel fpExampleMarkers); Firebase carriers restricted to endpoint URLs/tech names/evidence structured indicators or specific service-host values in evidence values. Candidate VALUES never copied into pack-authored fields (test-enforced; subject identity encoding is the asset model's own canonical form). RequiredAssetTypes single primary kind per rule (aws→secret_candidate, bucket→endpoint, firebase→evidence) documented under census gate AND-semantics following apis precedent; confidence tiered 0.9 exact-shape / 0.6 heuristic; bounded at 256; Config sortedKeys deterministic; context honored per iteration. Pipeline seam internal/pipeline/adapt/detect.go LoadCloudPack / NewDetectStageWithCloudPack / NewDetectStageWithAllPacks extended to 14 rules via ValidateRule→Register deepCopy→Validate→Seal, AllStages 12 unchanged, nil→empty unchanged; adapt/detect_seam_test.go counts 11→14 + cloud ID checks + secrets fixture + TestLoadCloudPackHelper. Tests hermetic: packs/cloud/cloud_test.go 13 tests (CheckAPIVersion, LoadsThroughSDK deepCopy+Seal incl RequiredAssetTypes aliasing, MetadataDepsCompat, RequiredAssetTypesSkipHonestly per-kind statuses, DetectorsHonorContext cancel→OutcomeCancelled, FailuresIsolated panic→failed not crashed, CacheColdWarmParity same FindingsTruncated byte-equal, DeterminismGolden, AWSKeyIndicator typed/evidence/example-suppression/mistyped-negative, BucketURL 4 shapes+provider split+negative, FirebaseIndicator 3 subjects+plain-prose negative, MixedPerRule, ConfigDeterministic sorted + disabled cache-hit parity); golden internal/detect/packs/cloud/testdata/cloud_report.golden -update twice md5-identical (aab4a3131642176c298aede0c55132df); gates gofmt clean, go vet ./... green, go build ./... green, go test ./... all pass, go test -race ./internal/detect/... ./internal/pipeline/adapt/... green, api_v1.golden untouched.
